...
The operational timeout defaults to five minutes and cannot be configured below four minutes. That minimum covers the worst-case 60-second blackbox scrape, the provisioned 30-second alert evaluation, Discord delivery, and the same scrape/evaluation path after recovery with margin. Only the loopback test seam may use a shorter timeout.
