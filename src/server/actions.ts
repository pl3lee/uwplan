...
export async function updateFreeCourseAction(
  courseItemId: string,
  filledCourseId: string,
) {
  try {
    const session = await auth();
    if (!session?.user) {
...
export async function removeCourseSelectionAction(courseId: string) {
  try {
    ...
    writeApplicationError("course-selection.remove.failed", error);
...
export async function createTemplateAction(template: CreateTemplateInput) {
  try {
    ...
    writeApplicationError("academic-plan.create.failed", error);
...
export async function renameTemplateAction(
  ...
  try {
    ...
    writeApplicationError("academic-plan.rename.failed", error);
    if (error instanceof Error) {
      return { error: { message: error.message } };
    } else {
...
export async function addCourseToScheduleAction(
  scheduleId: string,
  courseId: string,
  term: string,
) {
...
export async function removeCourseFromScheduleAction(
  scheduleId: string,
  courseId: string,
) {
  try {
...
export async function changeScheduleNameAction(
  scheduleId: string,
  name: string,
) {
  try {
...
export async function changeTermRangeAction(
  startTerm: Season,
  startYear: number,
  endTerm: Season,
  endYear: number,
...
export async function exportScheduleToCSV(scheduleId: string) {
  try {
    ...
    selectedCourses.forEach((course) => {
      csvContent +=
        escapeField(`${course.courseCode} - ${course.courseName}`) + "\n";
    });

    csvContent += "\nScheduled Courses:\n";

    const maxCourses = Math.max(
      ...Array.from(coursesByTerm.values()).map((courses) => courses.length),
    );
    csvContent +=
      Array.from(coursesByTerm.keys()).map(escapeField).join(",") + "\n";

    for (let i = 0; i < maxCourses; i++) {
      const row = Array.from(coursesByTerm.values())
        .map((courses) => courses[i] ?? "")
        .join(",");
      csvContent += row + "\n";
    }
